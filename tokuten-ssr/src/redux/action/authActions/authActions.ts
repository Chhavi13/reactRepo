...
import { get, setItemLocalStorage } from "../../../services/StorageService";
...
export const getPersonalDetailAction = () => (dispatch: any) => {
  ...
  return authService.getProfile().then((response: any) => {
    ...
    let userInfo: any = get('authData');
    userInfo = JSON.parse(userInfo);
    userInfo['profile_pic'] = response?.data?.data?.profile_image;
    setItemLocalStorage("authData", JSON.stringify(userInfo));
