...
export const MUserProfile: React.FC<IProps> = () => {
  ...
  let getId: any = param?.id ? parseInt(param?.id) : false;
  // for image cropper
  ...
  const getRendringData = async () => {
    try {
      if (!getId) {
        let data: any = JSON.parse(getItemLocalStorage('authData'));
        getId = data.id;
      }
      ...
      return {
        ...
        'title': `@${response?.data?.username} | tokuten`,
        'description': response?.data?.company_bio,
  ...
  let { data } = useQuery('data', getRendringData);
  ...
  const getUserDataByID = async () => {
    try {
      let response: any = await authService.getUserProfileDetail(getId);
      const media: any = response?.data?.user_media_files.map((item: any) => item = { 'file': item });
      setUserProfileData({ ...response.data });
      setPoints({ ...points, 'point': response.data.points })
  ...
  return (
    <div className="mobilemaincontainer">
      <MetaDataTags title={data?.title} description={data?.description} image={data?.image} url={`user/${data?.username}/${data?.id}`}/>
        ...
        <div className="profileheader_container user_image_profile1">
          ...
          <div className="profileimgmaincontainer">
            <p className="imagesshow">
              ...
              <LaunchOutlinedIcon className="icon_color" onClick={() => shareUrl(`user/${data?.username}/${data?.id}`)} />
              {getId && <MoreVertOutlinedIcon className="icon_color" onClick={() => { setPopup(true) }} />}
            </p>
          </div>
        </div>
            ...
            {getId && <div className="three_tabs_member pb-3">
              <AppBar position="static">
                ...
                >
                  <Tab icon={<StarsRoundedIcon />} aria-label="star" {...a11yProps(0)}
                    className={`${tier?.tier?.length === 0 && "d-none" ||
                      investerData?.investerdata?.length === 0 && tier?.tier?.length != 0 && "MuiTab-root-btntwo"}`}
                  />
              ...
              <TabPanel value={value} index={1}>
                <p className="subscribe-now-read-more">
                  <ReadMore>
                    {profileData?.subscript_description ? profileData?.subscript_dsescription : ''}
                  </ReadMore>
                </p>
                <br />
                <Button variant="primary" className={`subscribe-now-btn ${profileData?.me_subscribed && "subscribed"}`} onClick={() => profileData?.me_subscribed ? setUnSubscribe(!unSubscribe) : setSubscribeModal(!isSubscribeModal)} >
                  {profileData?.me_subscribed ? "Subscribed" : `Subscribe now for ${profileData.subscript_cost}/M`}
                </Button>
              </TabPanel>
              {/* subscribe now */}
