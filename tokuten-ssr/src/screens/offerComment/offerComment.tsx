...
export const OfferComment: React.FC<IProps> = ({ item, index, pageData, actualAPIFiles, base64ImagesURI, stepperCase, isPreview, setActiveStep, sliderImages, pageBack }: IProps) => {
  ...
  const getMetaDataForOffer = async () => { /* Get Meta data */
    try {
      let image: any, title: any, description: any, username: any, general_tag: any = '', primary_tag: any = '';
      let response: any = await getOfferListSingle(params.offer_Id);
      const metadata = response?.data?.data;
      if(metadata?.event_offer_images?.length > 0) image = metadata.event_offer_images[0].file;
      if(metadata?.offer_title) description = metadata.offer_title;
      if(metadata?.general_tag?.length > 0) general_tag = metadata.general_tag[0].name;
      if(metadata?.primary_tag?.name) primary_tag = metadata.primary_tag.name;
      if(metadata.username)    username = metadata.username;
      if(general_tag) title = `${general_tag} | `;
      if(primary_tag) title += `${primary_tag} | `;
      if(username)    title += `@${username} on tokuten`;
      const meta = { 'title': title, 'description': description, 'image': image, 'offer_id': metadata.id };
      return meta;
    } catch (error) {
      console.log(error);
    }
  ...
  let { data } = useQuery('data', getMetaDataForOffer);
    ...
    <div className="mobilemaincontainer content">
      <div className="mobile_container dash_tabs">
        <div className="sabrinamainscroll create_page">
          <MetaDataTags title={data?.title} description={data?.description} image={data?.image} url={`offer/comment/${data.offer_id}`} />
